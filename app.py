...
def set_plot_style(fig, height=None):
    layout_kwargs = dict(
        ...
        legend=dict(bgcolor="rgba(255,248,220,0.55)", bordercolor=LINE_COLOR, borderwidth=1,
                    font=dict(family="Gowun Batang, serif", size=12)),
        margin=dict(l=10, r=10, t=40, b=10),
    )
    if height:
        layout_kwargs["height"] = height
    fig.update_layout(**layout_kwargs)
    fig.update_xaxes(gridcolor=GRID_COLOR, linecolor=LINE_COLOR, tickfont=dict(family="Gowun Batang, serif", size=12))
    fig.update_yaxes(gridcolor=GRID_COLOR, linecolor=LINE_COLOR, tickfont=dict(family="Gowun Batang, serif", size=12))
    return fig
...
with tab1:
    ...
    with col1:
        ...
        keywords = ["글로벌 무역", "한국 경제", "GVC", "OECD 비교", "공급망", "무역 의존도", "데이터 시각화", "전략적 파트너"]
    ...
    with col2:
        ...
        fig_overview.update_layout(barmode="group", height=320)
        ...
        fig_radar.update_layout(
            polar=dict(
                radialaxis=dict(visible=True, range=[0, 100],
                                tickfont=dict(family="Gowun Batang, serif", size=10, color=FONT_COLOR),
                                gridcolor=GRID_COLOR),
                angularaxis=dict(tickfont=dict(family="Gowun Batang, serif", size=11, color=FONT_COLOR),
                                 gridcolor=GRID_COLOR)
            ),
            showlegend=True,
            paper_bgcolor=PAPER_BG,
            plot_bgcolor=PLOT_BG,
            font=dict(family="Noto Serif KR, Gowun Batang, serif", color=FONT_COLOR),
            legend=dict(bgcolor="rgba(255,248,220,0.55)", bordercolor=LINE_COLOR, borderwidth=1,
                        font=dict(family="Gowun Batang, serif", size=12)),
            height=320,
...
with tab6:
    ...
    with col_bar6:
        ...
        fig_partner = go.Figure(go.Bar(
            ...
            y=df_sorted["국가"],
            orientation="h",
            marker_color=[EXPORT_COLOR if c == "미국" else
                          "#7A5A3A" if c == "중국" else
                          IMPORT_COLOR if c == "베트남" else "#A07850"
